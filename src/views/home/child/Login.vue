<template>
	<el-button type='primary' @click='loginWindow'>登录账号</el-button>
</template>
	...
	loginWindow.webContents.on('did-navigate', (event, url) => {
		...
		loginWindow.webContents.session.cookies
			...
			.then((cookies) => {
				...
				;(ElNotification as any).success({ title: '成功', message: '账号已添加' })
			})
			.catch(() => {
				;(ElNotification as any).error({ title: '失败', message: '获取Cookie超时或者出现其他问题' })
			})
	})
}
